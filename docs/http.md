...
我们则需要来区分两种请求，来自网关的请求进行鉴权，而服务间的请求则直接调用。
可以给请求的header中添加一个参数用来区分。这样完全可以避免上面提到的问题。
这样我们总共实现了以下几个功能：

1. 服务间访问可以不鉴权，添加注解@Inner即可。
1. 网关访问不需要鉴权的资源，添加注解@Inner(value=false)即可。当然，这样服务间不鉴权也可以访问。
1. 为了安全性考虑，将网关中的请求头form参数清洗，以防有人模拟请求，来访问资源。

由于各个服务都是在内网环境中，只有网关会暴漏公网，因此服务间调用是没必要鉴权的。
