/*********************************************************
    > File Name: bmMatcher.cpp
    > Author: ims
    > Created Time: Mon Jun 29 22:57:19 2020
 *********************************************************/
...
void generateBadChar(const char *b, int m,
                     int *badchar) //(模式串字符b，模式串长度m，模式串的哈希表)
{
  int i, ascii;
  for (i = 0; i < SIZE; ++i) {
...
void generateGS(const char *b, int m, int *suffix,
                bool *prefix) //预处理模式串，填充suffix，prefix
{
  int i, j, k;
...
int moveByGS(int j, int m, int *suffix,
             bool *prefix) //传入的j是坏字符对应的模式串中的字符下标
{
  int k = m - 1 - j; //好后缀长度
  if (suffix[k] !=
      -1) // case1，找到跟好后缀一样的模式子串（多个的话，存的靠后的那个（子串起始下标））
    return j - suffix[k] + 1;
  for (int r = j + 2; r < m; ++r) // case2
  {
    if (prefix[m - r] ==
        true) // m-r是好后缀的子串的长度，如果这个好后缀的子串是模式串的前缀子串
      return r; //在上面没有找到相同的好后缀下，移动r位，对齐前缀到好后缀
  }
  return m; // case3,都没有匹配的，移动m位（模式串长度）
}
int str_bm(const char *a, int n, const char *b,
           int m) // a表示主串，长n; b表示模式串,长m
{
...
int main() {
  ...
  int posi = str_bm(a.c_str(), a.size(), b.c_str(), b.size());
  cout << a << "中第一次出现" << b << "的位置(从0开始)是：" << posi << endl;
  return 0;
}
