...
1. innodb，聚集索引，支持外键和事务(commit)、回滚(rollback)和崩溃修复能力(crash recovery capabilities)的事务安全(transaction-safe (ACID compliant))，不支持全文索引，不支持计数，统计的时候会遍历，
2. myisam，非聚集索引，不支持外键事务，支持全文索引，支持计数，查询效果较好
...
4.  Full-text索引
  ...
  [MySQL有哪些索引类型 ?](http://segmentfault.com/q/1010000003832312)
