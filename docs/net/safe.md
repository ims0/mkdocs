...
$data=$stime+$sign 传到后端，后端使用$stime与$token重新做md5，与md5做比较。


每次HTTP请求，都需要加上timestamp参数，然后把timestamp和其他参数一起进行数字签名。因为一次正常的HTTP请求，从发出到达服务器一般都不会超过60s，所以服务器收到HTTP请求之后，首先判断时间戳参数与当前时间相比较，是否超过了60s，如果超过了则认为是非法的请求。

### 基于nonce的方案
$once=md5($time.$ip.$mac)
...
### 防止 XSS 攻击
* 验证到达应用程序的输入：-必须验证从用户或外部应用程序进入 Web 应用程序的所有数据。这是最基本的预防方法。
* 在发送到浏览器之前对 HTML 进行编码：-为了帮助防止 XSS 攻击，应用程序需要确保页面中的所有变量输出在返回给最终用户之前都经过编码。
